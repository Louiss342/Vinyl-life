...
两处手绘页面的手写体，与设计稿 `Excalidraw/Drawing 2026-09-15 14.14.52`（专辑墙空态教程）
和 `Excalidraw/Drawing 2026-09-15 15.58.24`（设置面板）同源：
...
两个面按**两处文案**裁成子集：专辑墙空态教程（`src/core/i18n.ts` 的 `shelf.tutorial.*`）与
设置面板「关于」页（`src/core/about.ts` 的作者手记 + 词典里的版本 / 许可行 + 页面上写死的
拉丁文）。两处都是固定内容，所以字能全进子集；改完文案重跑脚本即可。
