...
export const DICT: Record<string, { zh: string; en: string }> = {
  ...
  // 导出笔记的骨架：摘要一行 / 概览 / 各分区标题与表头（表格用 Markdown 管道表）。
  // 不设 H1：笔记标题就是文件名；落点目录固定英文 Stats，不进词典（不随语言变）
  'stats.exportSummary': {
    zh: '> **{plays}** 次播放 · **{albums}** 张专辑 · **{tracks}** 首曲目 · {days} 天有记录',
    en: '> **{plays}** plays · **{albums}** albums · **{tracks}** tracks · {days} days with listening',
  },
  'stats.exportTotal': { zh: '- 累计播放：{n} 次', en: '- Total plays: {n}' },
  'stats.exportAlbums': { zh: '- 播放专辑：{n} 张', en: '- Albums played: {n}' },
  'stats.exportTracks': { zh: '- 播放曲目：{n} 首', en: '- Tracks played: {n}' },
  'stats.exportFirst': { zh: '- 首次记录：{date}', en: '- First play: {date}' },
  'stats.exportBusiest': {
    zh: '- 最活跃的一天：{date}（{n} 次）',
    en: '- Busiest day: {date} ({n} plays)',
  },
  'stats.exportMonthlyHeading': { zh: '## 近一年播放', en: '## Listening by month' },
  'stats.exportMonthlyHead': { zh: '| 月份 | 播放 | |', en: '| Month | Plays | |' },
  'stats.exportTopHeading': { zh: '## 播放最多', en: '## Most played' },
  'stats.exportTopHead': { zh: '| # | 专辑 | 播放次数 | 最近播放 |', en: '| # | Album | Plays | Last played |' },
  'stats.exportRemoved': { zh: '{title}（已移除）', en: '{title} (removed)' },
  'stats.exportRecentHeading': { zh: '## 最近播放', en: '## Recently played' },
  'stats.exportRecentHead': { zh: '| 专辑 | 播放次数 | 上次播放 |', en: '| Album | Plays | Last played |' },
  'stats.exportPropsHeading': { zh: '## 自定义统计', en: '## Custom statistics' },
  'stats.exportPropsHead': { zh: '| {name} | 播放次数 | 专辑数 |', en: '| {name} | Plays | Albums |' },
